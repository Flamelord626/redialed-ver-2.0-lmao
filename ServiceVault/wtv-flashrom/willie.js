...
if (romType === "US-LC2-disk-0MB-16MB-CPU5230") {
    romType = "JP-LC2-disk-0MB-8MB-CPU5230";
}
...
<p><br></p>
<center>
<h1>Welcome to Little Willie's<br>Flash ROM restarter!</h1>
</center>
<p><br></p>
<h2><font color="red">WARNING! This tool could very easily brick your box! It's recommended to be used <i>only</i> under the guidance of WebTV Redialed operators.</font></h2>
<br>
<ul>
<br>
<form action="upgrade-restart">
Build: <input type="text" id="build" name="build" size=5 maxlength=5 value="7352">
Type: 
...
<center><font size="1">(No provable affiliation to <a href="http://frogfind.com/read.php?a=http://en.wikipedia.org/wiki/Richard_Smalley">Richard Smalley</a>.)</font></center>
<font size="1"><font size="3"></font></font></li>
</body>
</html>`;
} else {
...
<td>
<br>
<shadow><font color=white><h1>&nbsp;Bootleg Willie's</h1></shadow></font>
</td>`;
...
&nbsp;Reverse sort&nbsp;<input type="checkbox" name="reverseSort"${query.reverseSort ? ' checked' : ''}>
</td>
<td align=right>
<input type="submit" text="#E7CE4A" value="Do it." borderimage="file://rom/borders/buttonborder2.bif">
    ...
    if (boxInfo && boxInfo.warning) {
        data += `<font color=red><b><p><u>WARNING: ${boxInfo.warning}.</u></b></font>`;
    }

    data += `<font color=e7ce4a><p>${boxInfo ? `Showing ${flashRoms.length} builds for <b>${boxInfo.romType}</b>.` : `we ain't got that`}
<br><br>`;
...
<td align=right><a href=>next</a></td>
    ...
    if (boxInfo) {
        data += `<table border align=center cellspacing=1 cellpadding=0>
<th>Num</th>
<th>Type</th>
<th>Parts</th>
<th>Size</th>
<th>Ver</th>`;
        ...
        flashRoms.forEach((details, index) => {
            yeah += `<tr>
<td align=center><a href="${boxInfo.romTypeShort == "BPS" && details.type == "internal-debug" ? 
    `client:showalert?message=You%20sure%20about%20this%20one%20chief%3F%3Cp%3E%3Cb%3Einternal-debug%3C%2Fb%3E%20builds%20are%20known%20to%20brick%20your%20type%20of%20box%20%28no%20more%20WebTV%21%29%3Cp%3ENote%3A%20WebTV%20Redialed%20is%20not%20liable%20for%20any%20damages%20caused%20by%20Halen-induced%20VCC%20shorts%20to%20ground.&buttonlabel2=Sounds%20good%20to%20me&buttonaction2=wtv-flashrom:/initiate-lc2-download?path=content/${boxInfo.romDir}/build${details.build}/${details.type}/${boxInfo.romType}-part000.rom&numparts=${details.parts}&buttonlabel1=Perhapsn%27t&buttonaction1=client:donothing` : 
    `wtv-flashrom:/initiate-lc2-download?path=content/${boxInfo.romDir}/build${details.build}/${details.type}/${boxInfo.romType}-part000.rom&numparts=${details.parts}`}">${details.build}</a></td>
<td>${details.type}</td>
<td>${details.parts}</td>
<td>${details.size}</td>
<td>${details.bString}</td>`;
            if (index + 1 !== Object.keys(flashRoms).length) yeah += `\n`; // Ensure we don't newline right before the end of the table
        });
